...
public class CommandLeetFishing implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        HashMap<Enum, Skill> skillTree = profile.getFishingSkillTree().getTree();
        ...
        if (args.length == 0) {
            ...
            //Check if first argument is a valid Fishing skill. If it is display the skill info if no other command follows.
            if (args.length == 1 && args[0] != null && !args[0].equals("")) {
                if (EnumUtilities.isInEnum(args[0], NormalSkillEnums.FishingSkillNames.class)) {
                    CommandUtils.sendSkillMessage(player,
                            profile.getFishingSkillTree()
                                    .getTree()
                                    .get(NormalSkillEnums.FishingSkillNames.valueOf(args[0])));
                } else {
                    ...
                    if (!EnumUtilities.isInEnum(args[0], NormalSkillEnums.FishingSkillNames.class)) {
                        ...
                        if (StringUtilities.isInteger(args[2])) {
                            if (SkillUtils.hasPrerequesiteSkill(
                                    ...
                                    profile,
                                    profile.getFishingSkillTree(),
                                    NormalSkillEnums.isFishingSkillEnum(args[0].toUpperCase().replace(' ', '_'))
                            )) {
        ...
        switch(args.length) {
            case 1:
                ...
                completions.addAll(Stream
                        .of(NormalSkillEnums.FishingSkillNames.values())
                        .map(NormalSkillEnums.FishingSkillNames::name).collect(Collectors.toList()));
