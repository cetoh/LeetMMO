...
public class CommandLeetHunting implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        HashMap<Enum, Skill> skillTree = profile.getHuntingSkillTree().getTree();
        ...
        if (args.length == 0) {
            ...
            //Check if first argument is a valid Hunting skill. If it is display the skill info if no other command follows.
            if (args.length == 1 && args[0] != null && !args[0].equals("")) {
                if (EnumUtilities.isInEnum(args[0], NormalSkillEnums.HuntingSkillNames.class)) {
                    CommandUtils.sendSkillMessage(player,
                            profile.getHuntingSkillTree()
                                    .getTree()
                                    .get(NormalSkillEnums.HuntingSkillNames.valueOf(args[0])));
                } else {
                    ...
                    if (!EnumUtilities.isInEnum(args[0], NormalSkillEnums.HuntingSkillNames.class)) {
                        ...
                        if (StringUtilities.isInteger(args[2])) {
                            if (SkillUtils.hasPrerequesiteSkill(
                                    ...
                                    profile,
                                    profile.getHuntingSkillTree(),
                                    NormalSkillEnums.isHuntingSkillEnum(args[0].toUpperCase().replace(' ', '_'))
                            )) {
        ...
        switch(args.length) {
            case 1:
                ...
                completions.addAll(Stream
                        .of(NormalSkillEnums.HuntingSkillNames.values())
                        .map(NormalSkillEnums.HuntingSkillNames::name).collect(Collectors.toList()));
